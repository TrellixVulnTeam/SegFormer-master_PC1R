...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        self.in_chans = in_chans*8*8

        self.DCT = DCTCompression(N=8,q=50)
        # patch_embed
        self.patch_embed1 = OverlapPatchEmbed(img_size=self.img_size, patch_size=7, stride=4, in_chans=self.in_chans,
                                              embed_dim=embed_dims[0])
        self.patch_embed2 = OverlapPatchEmbed(img_size=self.img_size // 4, patch_size=3, stride=2, in_chans=embed_dims[0],
                                              embed_dim=embed_dims[1])
        self.patch_embed3 = OverlapPatchEmbed(img_size=self.img_size // 8, patch_size=3, stride=2, in_chans=embed_dims[1],
                                              embed_dim=embed_dims[2])
        self.patch_embed4 = OverlapPatchEmbed(img_size=self.img_size // 16, patch_size=1, stride=1, in_chans=embed_dims[2],
